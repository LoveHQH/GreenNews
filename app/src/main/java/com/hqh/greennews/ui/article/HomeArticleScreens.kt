...
@Composable
private fun HomeArticleScreenWithList(
    uiState: ArticleUiState,
    showTopAppBar: Boolean,
    onRefreshPosts: () -> Unit,
    onErrorDismiss: (Long) -> Unit,
//    openDrawer: () -> Unit,
    ...
    hasPostsContent: @Composable (
        uiState: ArticleUiState.HasPosts,
        modifier: Modifier
    ) -> Unit
) {
    Scaffold(
...
//                    openDrawer = openDrawer,
        ...
        LoadingContent(
            empty = when (uiState) {
                is ArticleUiState.HasPosts -> false
                is ArticleUiState.NoPosts -> uiState.isLoading
            },
            ...
            content = {
                when (uiState) {
                    is ArticleUiState.HasPosts -> hasPostsContent(uiState, contentModifier)
                    is ArticleUiState.NoPosts -> {
